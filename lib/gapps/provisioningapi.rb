...
module GApps #:nodoc:
  ...
  class ProvisioningApi
    ...
    # Returns a boolean if a username exists
    # 	ex :	
    #			myapps = ProvisioningApi.new('root@mydomain.com','PaSsWoRd')
    #			exists = myapps.user_exists?('jsmith')
    def user_exists?(username)
      xml_response = @connection.request(@action[:user_retrieve], username)
      return (xml_response.nil?) ? false : true
    end
    ...
    def retrieve_all_users
      response = @connection.request(@action[:user_retrieve_all], nil)
    ...
    def retrieve_page_of_users(start_username)
      ...
      response = @connection.request(@action[:user_retrieve_all], param)
      user_feed = Feed.new(response.elements["feed"],  UserEntry)
    end
    ...
    def create_user(username, given_name, family_name, password, passwd_hash_function=nil, quota=nil)
      ...
      response = @connection.request(@action[:user_create], nil, msg.to_s)
    ...
    def update_user(username, given_name, family_name, password=nil, passwd_hash_function=nil, admin=nil, suspended=nil, changepasswd=nil, quota=nil)
      ...
      response = @connection.request(@action[:user_update], username, msg.to_s)
      user_entry = UserEntry.new(response.elements["entry"])
    end
    ...
    def delete_user(username)
      response = @connection.request(@action[:user_delete], username)
      return !response.nil?
    end

    # Returns a boolean if a NicknameEntry exists
    # 	ex :	
    #			myapps = ProvisioningApi.new('root@mydomain.com','PaSsWoRd')
    #			exists = myapps.nickname_exists?('jsmith')
    def nickname_exists?(nickname)
      xml_response = @connection.request(@action[:nickname_retrieve], nickname)
      return (xml_response.nil?) ? false : true
    end
    ...
    def retrieve_nicknames(username)
      xml_response = @connection.request(@action[:nickname_retrieve_all_for_user], username)
    ...
    def retrieve_all_nicknames
      xml_response = @connection.request(@action[:nickname_retrieve_all_in_domain], nil)
      nicknames_feed = Feed.new(xml_response.elements["feed"],  NicknameEntry)
      nicknames_feed = add_next_feeds(nicknames_feed, xml_response, NicknameEntry)
    end
    ...
    def create_nickname(username,nickname)
      ...
      response = @connection.request(@action[:nickname_create], nil, msg.to_s)
    ...
    def delete_nickname(nickname)
      response = @connection.request(@action[:nickname_delete], nickname)
      return !response.nil?
    end
    ...
    def retrieve_page_of_nicknames(start_nickname)
      ...
      xml_response = @connection.request(@action[:nickname_retrieve_all_in_domain], param)
      nicknames_feed = Feed.new(xml_response.elements["feed"],  NicknameEntry)
    end
    ...
    def create_group(group_id, properties)
      ...
      response = @connection.request(@action[:group_create], nil, msg.to_s)
    ...
    def update_group(group_id, properties)
      msg = RequestMessage.new
      msg.about_group(group_id, properties)
      response = @connection.request(@action[:group_update], group_id, msg.to_s)
      group_entry = GroupEntry.new(response.elements["entry"])
    end
    ...
    def delete_group(group_id)
      response = @connection.request(@action[:group_delete], group_id)
      return !response.nil?
    end
    ...
    def retrieve_groups(user)
      xml_response = @connection.request(@action[:groups_retrieve], user)
    ...
    def retrieve_all_groups
      xml_response = @connection.request(@action[:all_groups_retrieve], nil)
      list_feed = Feed.new(xml_response.elements["feed"], GroupEntry) 
      list_feed = add_next_feeds(list_feed, xml_response, GroupEntry)
    end
    ...
    def add_member_to_group(email_address, group_id)
      ...
      response = @connection.request(@action[:membership_add], group_id+'/member', msg.to_s)
    ...
    def remove_member_from_group(email_address, group_id)
      response = @connection.request(@action[:membership_remove], group_id+'/member/'+email_address)
      return !response.nil?
    end
    ...
    def is_member(email_address, group_id)
      begin
        xml_response = @connection.request(@action[:membership_confirm], group_id+'/member/'+email_address)
    ...
    def retrieve_all_members(group_id)
      xml_response = @connection.request(@action[:all_members_retrieve], group_id+'/member')
    ...
    def add_owner_to_group(email_address, group_id)
      ...
      response = @connection.request(@action[:ownership_add], group_id+'/owner', msg.to_s)
    ...
    def remove_owner_from_group(email_address, group_id)
      response = @connection.request(@action[:ownership_remove], group_id+'/owner/'+email_address)
      return !response.nil?
    end
    ...
    def is_owner(email_address, group_id)
      begin
        xml_response = @connection.request(@action[:ownership_confirm], group_id+'/owner/'+email_address)
        # if the email_address is not member of the group, an error is raised, otherwise true is returned 
        return true
      rescue GDataError => e
        return false if e.reason.eql?("EntityDoesNotExist")
    ...
    def retrieve_all_owners(group_id)
      xml_response = @connection.request(@action[:all_owners_retrieve], group_id+'/owner')
